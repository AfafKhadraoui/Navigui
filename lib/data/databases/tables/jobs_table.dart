...
class DBJobsTable extends DBBaseTable {
  ...
  Future<List<Map<String, dynamic>>> getActiveJobs() async {
    try {
      ...
      final totalJobs = await db.rawQuery('SELECT COUNT(*) as count FROM $tableName');
      print('🔍 Total jobs in database: ${totalJobs.first['count']}');
      
      // Debug: Check jobs with status
      final statusJobs = await db.rawQuery("SELECT COUNT(*) as count FROM $tableName WHERE status = 'active'");
      print('🔍 Jobs with status=active: ${statusJobs.first['count']}');
      
      // Debug: Check jobs with is_draft
      final draftJobs = await db.rawQuery("SELECT COUNT(*) as count FROM $tableName WHERE is_draft = 0");
      print('🔍 Jobs with is_draft=0: ${draftJobs.first['count']}');
      ...
      print('🔍 Query returned ${result.length} jobs');
      return result;
    } catch (e, stackTrace) {
